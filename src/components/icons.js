/* Magazine Grid */
export const IconMagazineGrid = <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
	width="96" height="96" viewBox="0 0 96 96" preserveAspectRatio="xMidYMid meet">
	<g transform="translate(0,96) scale(0.1,-0.1)" fill="#1a6bb2">
		<path d="M40 650 l0 -130 120 0 120 0 0 99 c0 54 3 113 6 130 l6 31 -126 0 -126 0 0 -130z" />
		<path d="M360 650 l0 -130 120 0 120 0 0 99 c0 54 3 113 6 130 l7 31 -127 0 -126 0 0 -130z" />
		<path d="M680 650 l0 -130 120 0 120 0 0 130 0 130 -120 0 -120 0 0 -130z" />
		<path d="M40 310 l0 -130 126 0 c126 0 126 0 120 23 -3 12 -6 70 -6 130 l0 107 -120 0 -120 0 0 -130z" />
		<path d="M360 310 l0 -130 126 0 c126 0 126 0 120 23 -3 12 -6 70 -6 130 l0 107 -120 0 -120 0 0 -130z" />
		<path d="M680 310 l0 -130 120 0 120 0 0 130 0 130 -120 0 -120 0 0 -130z" />
	</g>
</svg>;

/* Magazine List */
export const IconMagazineList = <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
	width="96" height="96" viewBox="0 0 96 96" preserveAspectRatio="xMidYMid meet">
	<g transform="translate(0,96) scale(0.1,-0.1)" fill="#1a6bb2">
		<path d="M40 650 l0 -130 120 0 120 0 0 130 0 130 -120 0 -120 0 0 -130z" />
		<path d="M360 650 l0 -130 280 0 280 0 0 130 0 130 -280 0 -280 0 0 -130z" />
		<path d="M40 310 l0 -130 120 0 120 0 0 130 0 130 -120 0 -120 0 0 -130z" />
		<path d="M360 310 l0 -130 70 0 c40 0 160 0 280 0 l210 0 0 130 0 130 -280 0 -280 0 0 -130z" />
	</g>
</svg>;

/* Magazine Vertical */
export const IconMagazineVertical = <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
	width="96" height="96" viewBox="0 0 96 96" preserveAspectRatio="xMidYMid meet">
	<g transform="translate(0,96) scale(0.1,-0.1)" fill="#1a6bb2">
		<path d="M40 480 l0 -290 200 0 200 0 0 290 0 290 -200 0 -200 0 0 -290z" />
		<path d="M520 700 l0 -70 200 0 200 0 0 70 0 70 -200 0 -200 0 0 -70z" />
		<path d="M520 480 l0 -70 200 0 200 0 0 70 0 70 -200 0 -200 0 0 -70z" />
		<path d="M520 260 l0 -70 200 0 200 0 0 70 0 70 -200 0 -200 0 0 -70z" />
	</g>
</svg>;

/* Number Two */
export const IconNumberTwo = <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
	<path d="M0 0h24v24H0z" fill="none" />
	<path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-4 8c0 1.11-.9 2-2 2h-2v2h4v2H9v-4c0-1.11.9-2 2-2h2V9H9V7h4c1.1 0 2 .89 2 2v2z" />
</svg>;
// Source: https://material.io/tools/icons/?icon=looks_two&style=baseline

/* Number Three */
export const IconNumberThree = <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
	<path d="M.01 0h24v24h-24z" fill="none" />
	<path d="M19.01 3h-14c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-4 7.5c0 .83-.67 1.5-1.5 1.5.83 0 1.5.67 1.5 1.5V15c0 1.11-.9 2-2 2h-4v-2h4v-2h-2v-2h2V9h-4V7h4c1.1 0 2 .89 2 2v1.5z" />
</svg>;
// Source: https://material.io/tools/icons/?icon=looks_3&style=baseline

/* Number Four */
export const IconNumberFour = <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
	<path d="M0 0h24v24H0z" fill="none" />
	<path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-4 14h-2v-4H9V7h2v4h2V7h2v10z" />
</svg>;
// Source: https://material.io/tools/icons/?icon=looks_4&style=baseline
